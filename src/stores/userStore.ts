...
class UserStore {
    ...
    async loginGoogle(role: string) {
        try {
            ...
            const response = await axiosInstance.post('/api/account/loginGoogle', form);
            if (response.data) {
                if (response.data.user) {
                    runInAction(() => {
                        this.user = response.data.user;
    ...
    async updateCandidateProfile(updatedList: UpdatedList) {
        try {
            const response = await api.put('/api/account/updateCandidate', updatedList);
    ...
    async updateEmployerProfile(updatedList: UpdatedList) {
        try {
            const response = await api.put('/api/account/updateEmployer', updatedList);
